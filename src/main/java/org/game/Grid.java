...
public class Grid implements ISubscriber {
    ...
    public Grid(int rows, int columns, double seedingPercentage){
        if (rows <=0 || columns <=0 || seedingPercentage<=0) throw new InvalidParameterException("A grid cannot be created with a non-positive row, column or seedingPercentage");
        if (seedingPercentage>=1) throw new InvalidParameterException("A grid cannot be created with a seedingPercentage beyond the range of 0-1");
        this.rows = rows;
    ...
    public void onEvent(Event event, Object o) {
        ...
        switch (event){
            case Event.CELL_DESTROY:
                this.grid[row][column] = new Space(row, column);
                this.cellsCount--;
